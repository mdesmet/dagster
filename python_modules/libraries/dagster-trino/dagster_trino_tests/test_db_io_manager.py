...
import pytest
from dagster_trino import DbTypeHandler
from dagster_trino.db_io_manager import DbClient, DbIOManager, TablePartition, TableSlice
...
def test_asset_out():
    ...
    table_slice = TableSlice(catalog="database_abc", schema="schema1", table="table1")
...
def test_asset_out_columns():
    ...
    assert handler.handle_input_calls[0][1] == TableSlice(
        catalog="database_abc", schema="schema1", table="table1", columns=["apple", "banana"]
    )
